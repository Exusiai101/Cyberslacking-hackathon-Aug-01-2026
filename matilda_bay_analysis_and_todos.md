...
- [ ] **Marketplace UI & Trading Terminal (`dashboard.html` & `dashboard.js`)**:
  - Add a **"Matilda Bay Marketplace"** tab to the dashboard displaying:
    ...
    4. **Council Subsidy Grant Tool** to inject credits into Pod 4 (Reed's End).
...
To build trust across isolated pods after a disaster, **every resource metric, council allocation, and marketplace trade must operate on a 100% transparent system**. 
...
1. **Public Pod Telemetry & Runway Meters**:
   - Every pod's exact stockpiles, population, vulnerability headcount, and runway days are displayed publicly.
   - **Exposes Hoarding**: Pod 3 (Tallowfen)'s massive food surplus is visible to all, encouraging trade rather than secretive stockpiling.
...
3. **Public Trade Ledger & Order Book**:
   - All trade offers (`TradeOffer`) and credit balances (`PodWallet`) are open to the entire bay, preventing price gouging or secret backroom deals.

4. **Allocation Audit Trail**:
...
4. Build **Phase 5** Trading Marketplace & Currency System (`PodWallet`, `TradeOffer`, and Marketplace UI).
5. Enforce **Transparency Principles** across all API responses, dashboards, and transaction ledgers.
